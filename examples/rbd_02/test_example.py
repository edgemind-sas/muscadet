...
from examples.rbd_02.system import my_rbd
...
def test_component_values():
    assert all(my_rbd.indicators["S_is_ok_fed_out_mean"].values["values"] == 1)
    assert all(my_rbd.indicators["B1_is_ok_fed_out_mean"].values["values"] == 1)
    assert all(my_rbd.indicators["B2_is_ok_fed_out_mean"].values["values"] == 1)
    assert all(my_rbd.indicators["T_is_ok_fed_in_mean"].values["values"] == 1)
import pytest
