...
- Nothing in PyCATSHOO requires equation methods to be registered during component construction. Every vendor sample does register there, but the manual states no freeze point, which is what makes KTD14's first-run registration viable. U6 settles it.
- PDMP managers sharing one name across components aggregate into a single equation system, so the order integers sequence equations across components rather than only within one. The manual's manager-lookup wording implies it and the vendor samples rely on it, but the engine is a binary distribution and this is not provable from source. U7 verifies it on a two-component model before the ordering work depends on it.
