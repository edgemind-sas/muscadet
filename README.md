...
**What this change does not close.** The scale above bounds the demand sweep only. A component carrying **two rule sets on one input** sizes each set in the production sweep from the shared input budget, without reading that scale back, so a set whose outlets ask for nothing still runs, eats the budget the other set needed, and drops what it makes. Measured on a supply of 5: the first-declared set ran at 4 and dropped 4 an hour while the second, the only one with a real consumer, produced nothing. Pre-existing, and reached more often now: it used to take every outlet of a set being asked nothing, it now takes one.
