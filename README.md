...
Until 3.0.0 the scale was a **maximum** over what the rule's outputs were asked for. Only models with a **multi-output rule whose connected outputs are asked for unequal quantities** change, and they change in the direction of conservation. A single-output rule, or one whose outputs are all asked the same, is byte-identical.

Neither the muscadet suite (1180 tests) nor the COD3S Platform continuous suite (538 tests) nor the H2 parity bench had such a model, so nothing in either repository moved: the semantics only ever bit where nobody had looked. Check your own models for the shape.
