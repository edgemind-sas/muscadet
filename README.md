...
- **The least demanded output sets the scale.** A rule runs at the scale its **most constrained** output allows, never at the one its most demanding output would need: the outputs of a rule are correlated by construction, so running above what any one of them can take produces a surplus of the others that has nowhere to go, and nothing in the model records where it went. Two consumers asking 10 and 2 of a rule producing both in equal parts get 2 each, and the bigger one goes short: serving it would take making 8 units of the other product for a consumer that cannot take them. This was a **maximum** until 3.0.0; see the migration note below.
- **What the outputs are asked for sets how much is claimed upstream.** A component claims from its inputs only what its outputs are actually asked for, mapped back through the rule's declared coefficients. An output **nothing is connected to** asks for nothing, so it constrains nothing: a deliberately unwired output — a vent, a discharge, a branch not built yet — is a legitimate model and neither raises nor makes the component draw more. Two exceptions worth knowing: an unwired output with a **capacity** behind it does constrain, at the fill rate the volume claims for itself, which is 0 by default; and a branch that ends in an unwired **pipe** rather than an unwired output is a dead end, not a vent, and stops the rule upstream. A consumer connected and asking for **zero** is a different statement and does constrain, at scale zero. When *no* output of a rule constrains it — every one of them unwired — the rule runs at its **nominal** scale, claiming exactly its declared `cons` coefficients. Wire a consumer, and declare what it wants, whenever a scenario depends on a rule running above nominal. The one thing that still asks with nobody connected is a **capacity** sitting behind the output: its `fill_rate` is a claim the volume makes for itself, so a buffered output carries it whether or not anything is wired to it — and what is produced into that volume stays there instead of leaving through a connection that does not exist. A **full** buffer claims nothing, which under the rule above is what stops the rule filling it: an electrolyser whose hydrogen outlet is blocked runs until its pressure buffer is full, then stops, and stops making oxygen too.
...
- **Two rule sets on one input share it.** Each is sized against what the previous ones left, in **declaration order**: the first set declared is the first served. Without that they were each told the whole of the input and produced twice what the component received. Each is *also* capped by what its own outputs are asked for, so a set nothing is asking anything of takes none of the shared budget rather than taking it and dropping what it makes.
- **A loss is not an absent outlet**, and the two must not be harmonised. A **derating** that cuts an output to zero does *not* cut the reagent draw: the product is made and the fault destroys it on the way out, so sparing the reagents would let the surviving outputs be made out of nothing. That is why the derating factor is composed by *maximum* over a rule's outputs while the scale above is a *minimum*. A demand of zero is the opposite statement: no fault, no outlet, and nothing may be created that has nowhere to go.
...
#### Upgrading from 2.x: a rule runs at its least demanded output

Until 3.0.0 the scale was a **maximum** over what the rule's outputs were asked for. A **single-output** rule is byte-identical. Everything else deserves a look, and the criterion is narrower than it sounds: what is compared is `demand / coefficient`, not the demand, so two outlets asked for the same quantity still weigh differently as soon as their coefficients differ. `1 feed -> 2 a + 1 b` with both consumers asking 4 ran at 4 and now runs at 2.

Three shapes change, all toward conservation:

- a multi-output rule whose **connected** outlets are asked unequally, the ratio above included;
- an outlet whose downstream saturates while a sibling outlet keeps asking, which is the same thing arriving later: a by-product buffered on its way to a slow consumer now throttles the main product when its buffer fills;
- an **unwired** outlet with a **capacity** behind it. A capacity's `fill_rate` defaults to 0, a pure buffer that claims nothing, so such an outlet publishes a hard zero and its rule stands still from t=0 where it used to run and drop everything it made. The only shape in which an unwired outlet constrains at all.

Neither the muscadet suite nor the COD3S Platform backend suite (7316 tests) nor the H2 parity bench had any of the three, so nothing in either repository moved: the semantics only ever bit where nobody had looked. Note what that says about the parity bench in particular, whose rule *is* multi-output: it is unaffected because its oxygen outlet is wired to nothing, and an unwired outlet without a capacity is dropped from the scale entirely.

What it was doing, measured. An electrolyser making 2 hydrogen and 1 oxygen per unit of power, its hydrogen outlet wired to a tank asking for nothing and its oxygen outlet to a discharge:

| | Before (maximum) | After (minimum) |
|---|---|---|
| Power drawn | 1.0, indefinitely | 0.0 |
| Oxygen vented | 1.0/h, indefinitely | 0.0 |
| Hydrogen delivered | 0.0 | 0.0 |
| Hydrogen unaccounted for | **2.0/h** | none |

Put a 10-unit pressure buffer behind that blocked outlet and the loss stops being an accounting matter: the buffer reached **39.999 units in twenty hours** and went on climbing, past the volume it was declared with. Block the oxygen outlet too and everything was already exact, buffer resting on 10.000 and draw at 0.000. One outlet still asking was the whole of the defect.

**If a model relied on the old behaviour**, it was relying on an outlet discharging freely, and that is now said rather than inferred: leave the outlet unconnected, or wire it to a consumer asking for more than the rule can make. Both were already the recommended way to declare a vent, and the second only started working properly in 3.0.0.

**One shape cannot say it, and it is worth knowing before you meet it.** A rule-less pass-through carries the demand of whatever is beyond it and has no demand default of its own, so a branch that ends in a **pipe** wired to nothing publishes zero and stops the rule upstream, main product included: a reactor delivering 10 of its useful product drops to 0, silently. An unwired *output* is a modelled vent; an unwired *pipe* is a dead end. Wire a discharge at the end of that pipe, or remove the pipe.

**The production sweep obeys that scale too, since 3.1.0.** It used to size each rule set from the shared input budget alone, without reading the scale back, and that is equivalent only when the demand sweep has already narrowed the input to what the outputs want. Two shapes escaped it:

- **Two rule sets on one input.** The budget is then the aggregate, so a set the demand sweep had stopped ran anyway, ate the budget its sibling needed, and dropped what it made. Measured on a supply of 5: the first-declared set ran at 4 and dropped 4 an hour while the second, the only one with a real consumer, produced nothing, and swapping the declaration order swapped which consumer was served. Declaration order stays the priority order over a contested budget; it no longer decides whether anything is dropped.
- **A rule that consumes nothing**, which is a *single*-set change and the one to check your models for: a source rule with no `cons` map, or the catalyst idiom whose only coefficient is 0. There is no reagent to size against, so the demand never reached the production. Such a rule now delivers what its outputs ask for instead of its nominal rate. A 2 kW generator feeding a 0.5 kW load produces **0.5**, where it used to produce 2.0 and push 1.5 into a buffer that claims nothing: `fill_rate` defaults to 0. Declare the fill rate the buffer really has and the charging comes back, stated rather than inferred.

**What this does not close: two rule sets producing into ONE output.** Each set is handed the whole of that output's demand and nothing apportions it, so the mirror image of the shared input budget is still open on the output side. Measured, two sets each making `x` for a consumer asking 3: production 6.0, delivery 3.0, three an hour drawn from the suppliers and dropped. Closing it takes an apportionment policy over a contested output, which the input side has and the output side does not.
...
What stays open is narrower and worth naming: a discharge thresholded on a rate **arriving at its own component**. The comparison and the command then sit on one component, so no edge of the graph runs between them and no signal leaves; what closes is the demand a volume publishes upstream, which is capped by what it may release. Such a montage admits two fixpoints when the threshold sits between the regimes, and the solver keeps whichever the first evaluation reached. Threshold a **level** and the question does not arise: a level is integrated, and integrated state is what breaks a loop.

Commanding a discharge from a measure of **production** stays legitimate, and that is why the shape is not forbidden outright: a battery helping a plant on one bus, ordered by a threshold on what the plant delivers, builds. The two producers are siblings, so the order never reaches what the threshold reads.
...
**A sourced publication reports its reading at instant 0**, and at the start of every Monte Carlo sequence, exactly like a capacity's own level. The equation that keeps it current does not run at that instant: the engine samples it first and evaluates afterwards, so until 4.3.0 an instrument standing in front of a full tank announced its declared default there. The same holds for the `publish` channel of the shipped `SensorContinuous`, which compiles to one of these.

**And it is ordered by the wiring, not by the order it was written in.** A sourced publication takes its equation from the same band, and the same topological sort, as a controller's `"value"` output: instruments and controllers are one population here, because the question the sort answers is whether a reading has settled, and that does not depend on the class publishing it. So a chain of instruments, or a chain that alternates the two, settles in one evaluation however it was declared. Until 4.4.0 an instrument took its integer from a band of its own below the controllers, allocated in declaration order, so an instrument reading a controller was refreshed before it and a relay declared before the instrument it reads was permanently one evaluation behind. See [what an output carries at instant 0](#what-an-output-carries-at-instant-0).
...
**A seed takes the order its own equation takes**, and since 4.4.0 that is one order for the two natures of republication: a controller's `"value"` output and an `ObjFlow`'s sourced `MeasurementOut` are sorted together, so a chain that alternates them settles whole at instant 0 and in one evaluation afterwards. What a montage cannot do is close on itself, which is refused at build.
...
A **cycle between republishers is refused**, with a `muscadet.ControllerSignalCycleError`, at the first run and before a single equation is registered. One controller's value output is another's observation input, so a chain of them has exactly one evaluation order, upstream first, and a chain closing on itself has none. Republishers take their integers from a band of their own above the capacities, and within that band from a topological sort of the signal graph, so a chain of three settles in one evaluation whatever order the components were declared in. Ties break on declaration order. The graph holds `ObjFlow` instruments as well as controllers since 4.4.0, so a loop closing through an instrument is refused too, where it used to build.

**The refusal is per COMPONENT, not per channel**, and widening the graph widened what that costs. Two nodes that read each other on channels forming no loop are refused all the same: a controller publishing a reading, an instrument relaying it, and the same controller alarming on the relayed value is an ordinary self-monitoring montage whose channels are acyclic, and it no longer builds. Split the controller in two, or read the instrument from a third one. The granularity is the one controllers have always been refused at; what changed is the population it applies to.
...
- **A signal cycle is refused per component, so an acyclic set of channels can be refused with it.** The sort is over components, and two nodes republishing into each other are a loop of two whatever channels carry the two publications. Since 4.4.0 the population includes the `ObjFlow` instruments, so the shape this newly costs is real: a controller watching, through an instrument, a reading it publishes itself. Refusing wrongly is worse than missing, and this is the one place the module does it; a per-channel graph is what would close it.
