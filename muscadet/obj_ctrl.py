...
* ``controls_in`` -- **observation inputs**. An input is a measurement channel,
  ...
  edge to the graph the acyclicity check walks.
...
**Scope of this module today.** It is the skeleton: a controller declares,
builds and connects. The aggregation of several sources on one input, the
closed output grammar (aggregate, compare, band, combine, republish) and the
...
from .capacity import MeasurementIn, MeasurementOut
...
#: Declaration keys an OBSERVATION INPUT reads, forwarded verbatim to
#: :class:`muscadet.MeasurementIn`. ``combine`` and ``combine_fun`` are
#: deliberately absent: how several sources reduce to one value is the
#: aggregation the controller declares, not a property of the wire, and
#: admitting them here would open a second door onto it.
CONTROL_IN_KEYS = ("kind", "flows", "level_default", "fill_default", "rate_default")
...
class ObjCtrl(cod3s.PycComponent):
    ...
    controls_in : list of dict, optional
        Observation inputs, each ``{"name": ..., **CONTROL_IN_KEYS}``.
    ...
    def add_control_in(self, name, **params):
        """Declare one observation input (R4).
        ...
        **params : dict
            :data:`CONTROL_IN_KEYS`, forwarded to
        ...
        ------
        ValueError
            On an unknown declaration key, a name already taken, or a message
        ...
        channel = MeasurementIn(name=name, **params)
